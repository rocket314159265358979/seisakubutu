document.addEventListener("DOMContentLoaded", () => {
    ...
    // パワーアップの種類
    const powerUpTypes = [
        { type: "speed", color: "green", effect: () => { player.speed += 3; }, duration: 5000, message: "Speed Up!" },
    ...
    function shoot() {
        const now = Date.now();
        const fireRate = player.fastReload ? 100 : (player.rapidFire ? 0.0002 : 0.00005);
        ...
        if (now - player.lastShot >= fireRate) {
            ...
            if (player.doubleShot) {
                bullets.push({ x: bulletX - 10, y: player.y, width: 5, height: 10, speed: 10 });
                bullets.push({ x: bulletX + 10, y: player.y, width: 5, height: 8 b10, speed: 10 });
            }
    ...
    function spawnPowerUp(enemyX, enemyY) {
        if (Math.random() < 1000) { // パワーアップの生成確率
